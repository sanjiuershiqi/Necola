...
----
   an open-source multi-feature plugin for Left 4 Neko (L4N)
</div>

## Necola-ADS (L4N 插件版)

本仓库已改造为 **L4N (Left 4 Neko) 附属插件**，提供 ADS、主题反馈、伤害 UI、战役计时和可扩展菜单模块。
...
- `OnGameLaunch` 或任意 `OnModuleLoaded` 回调中先到者通过原子门控启动初始化线程;线程每秒检查 9 个 Source 模块。线程创建失败允许后续回调重试,模块等待超时或必需接口/pattern/Hook 缺失时会记录原因并保持插件停用。
...
│   ├── Hooks.cpp      # 5 个 Raw hook 组(当前 12 个 MinHook detour)
│   ├── Vars.h         # ADS 相关变量
